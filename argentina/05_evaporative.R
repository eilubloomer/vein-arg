...
maxage   <- 40
metadata[family == "PC"    , maxage := maxage]
metadata[family == "LCV"   , maxage := maxage]
metadata[family == "TRUCKS", maxage := maxage]
metadata[family == "BUS"   , maxage := maxage]
metadata[family == "MC"    , maxage := maxage]
...
metadata_evap <- metadata[metadata$fuel_eea_old %in% "G" & metadata$v_eea_old %in% c("PC", "LCV", "Motorcycle"), ]
...
for(k in seq_along(reg)) {
  ...
  for (i in seq_along(metadata_evap$vehicles)) {
    ...
    cat("\n", metadata_evap$vehicles[i], rep("", max(nchar(metadata_evap$vehicles) + 1) - nchar(metadata_evap$vehicles[i])) )
    ...
    # euro
    cate <- suppressWarnings( as.character(as.roman(gsub("Euro ", "",  euro[[metadata_evap$vehicles[i]]]))))
    cate[is.na(cate)] <- "PRE"
    ...
    ef <- ef_evap(ef = evrl[1:ncol(x)],
                  ...
                  ca = ifelse(metadata_evap$cc_eea_old[i] == "<=1400", "small",
                              ifelse(metadata_evap$cc_eea_old[i] == ">2000", "large",
                                     ifelse(metadata_evap$cc_eea_old[i] == "1400_2000", "medium", "no"))),
                  show = F,
                  kmday = weighted.mean(mileage[[metadata_evap$vehicles[i]]]/365, as.numeric(x)), pollutant = "NMHC")

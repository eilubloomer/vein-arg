...
fuel_spec <- readRDS("config/fuel_spec.rds"); setDT(fuel_spec)
fuel      <- readRDS("config/fuel.rds"     ); setDT(fuel     )

reg <- unique(fuel$region)
pol <- c( "CO", "HC", "NMHC", "NOx", "CO2", "PM", "NO2", "NO", "CH4")

suppressWarnings(file.remove("emi/exhaust.csv"))

cat("Estimating emissions Hot Running\n")

metadata_original <- metadata                                  #save copy of metadata
metadata <- metadata[metadata$fuel != "ELEC", ]                #pull out ELECTRIC vehicles
...
fcorr<- fuel_corr(
  euro = euros,
  g = c(e100 = fc$G_e100, aro = fc$G_aro,  o2 = fc$G_o2, e150 = fc$G_e150, olefin = fc$G_olefin, s = fc$G_s),
  d = c(den = fc$D_den, pah = fc$D_pah, cn = fc$D_cn, t95 = fc$D_t95, s = fc$D_s)
) 
...
for(k in seq_along(reg)) {
  ...
  for (i in seq_along(metadata$vehicles)) {
    ...
    for (j in seq_along(pol)) {
      ...
      f_fcorr <- fcorr[veh == metadata$v_eea_old_fuel[i] & pol == pol[j]]
